...
def analyze_text(text: str) -> Dict[str, Any]:
    ...
    avg_sent_len = sum(len([t for t in s if not t.is_punct]) for s in sentences) / max(
        1, len(sentences)
    )
...
PROMPT_TEMPLATE = (
    "You are a professional editor. Rewrite the following passage to improve grammar, clarity, and conciseness, preserving the original meaning. "
...
def _call_openai_for_chunk(chunk: str) -> str:
    ...
    resp = openai.ChatCompletion.create(
        model="gpt-3.5-turbo", messages=messages, temperature=0.2, max_tokens=1500
    )
